...
// let blackList = [] //存放黑名單
let blackList = JSON.parse(localStorage.getItem("blackFriend"))
...
searchForm.addEventListener("submit", function onSearchFormSubmited(event) {
  ...
  fliterFriends = blackList.filter(friend => friend.name.toLowerCase().includes(keywords))
...
function removeDislike(id) {
  ...
  const dislikeIndex = blackList.findIndex((friend) => friend.id === id)
  // console.log(movieIndex)

  blackList.splice(dislikeIndex, 1)
  localStorage.setItem("blackFriend", JSON.stringify(blackList))
  displayAllCards(blackList)
